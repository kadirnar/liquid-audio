...
For quality-oriented ASR and omni training, including LoRA, SpecAugment, task-balanced sampling, resumable
checkpoints, and before/after WER evaluation, see the complete [training guide](docs/TRAINING.md). Reproducible starter
recipes are provided in [`configs/asr_lora.json`](configs/asr_lora.json) and
[`configs/omni_lora.json`](configs/omni_lora.json).

```bash
make setup
make check
make train-asr TRAIN_DATA=data/asr/train VAL_DATA=data/asr/validation
```
