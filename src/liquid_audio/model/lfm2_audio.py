...
from liquid_audio.decoding import TextDecodingConfig, has_repetition_loop, prepare_text_logits
...
class LFM2AudioModelOutput:
    ...
    text_loss_weight: torch.Tensor | None = None
    audio_loss_weight: torch.Tensor | None = None
...
class LFM2AudioModel(nn.Module):
    ...
    def save_config(self, output_dir: str | Path) -> None:
        """Write the model architecture config next to a saved state dict."""

        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)
        config = asdict(self.conf)
        config["lfm"] = self.conf.lfm.to_dict()
        with (output_path / "config.json").open("w", encoding="utf-8") as stream:
    ...
    def generate_sequential(
        ...
        *,
        ...
        text_decoding: TextDecodingConfig | None = None,
        ...
        decoding = text_decoding or TextDecodingConfig()
        decoding.validate()
        ...
        for _ in range(max_new_tokens):
            ...
            if current_modality == LFMModality.TEXT:
                ...
                text_logits = prepare_text_logits(text_logits, text_history, decoding)
                ...
                if has_repetition_loop(
                    text_history,
                    ngram_size=decoding.loop_ngram_size,
                    repeats=decoding.max_consecutive_loop_repeats,
                ):
                    next_token = next_token.new_tensor([decoding.eos_token_id])
                yield next_token

                if int(next_token.item()) == 128:  # <|audio_start|>
                    current_modality = LFMModality.AUDIO_OUT
                if int(next_token.item()) == decoding.eos_token_id:  # <|im_end|>
                    break
        ...
        *,
        ...
        for _ in range(max_new_tokens):
            ...
            if current_modality == LFMModality.TEXT:
                ...
                if int(next_token.item()) == 130:  # <|text_end|>
                    text_done = True
    ...
    def logits(
        ...
        if audio_output_embeddings.shape[0] == 0:
            audio_logits = out_emb.new_empty((0, self.audio_vocab_size))
            return text_logits, audio_logits, shifted_text_tokens, shifted_audio_tokens.reshape(-1)
    ...
    def forward(
        ...
        *,
        ...
        text_weights = torch.ones_like(text_loss)
        if end_token_weight != 1.0:
            text_weights = torch.where(text_labels == 7, end_token_weight, text_weights)
        ...
        audio_loss = nn.functional.cross_entropy(
            ...
            ignore_index=-100,
            reduction="none",
            label_smoothing=audio_label_smoothing,
        )
        ...
        text_weight_sum = text_weights.sum()
        resolved_text_multiplier = self.conf.text_loss_multiplier if text_loss_multiplier is None else text_loss_multiplier
        resolved_audio_multiplier = self.conf.audio_loss_multiplier if audio_loss_multiplier is None else audio_loss_multiplier
        weighted_tokens = resolved_text_multiplier * text_weight_sum + resolved_audio_multiplier * audio_tokens

        text_loss_sum = (text_loss * text_weights).sum()
        loss = (resolved_text_multiplier * text_loss_sum + resolved_audio_multiplier * audio_loss.sum()) / (
        ...
        return LFM2AudioModelOutput(
            ...
            text_loss=text_loss_sum / (text_weight_sum + 1e-6),
            audio_out_tokens=torch.tensor(audio_tokens, device=loss.device),
            text_tokens=torch.tensor(text_tokens, device=loss.device),
            audio_in_tokens=mel2emb_len(batch.audio_in_lens).sum(),
            loss_weight=weighted_tokens.detach(),
            text_loss_weight=text_weight_sum.detach(),
            audio_loss_weight=torch.tensor(audio_tokens, device=loss.device),
        )
